...
export function CardVisualCompact({ card, locale = 'en' }: CardVisualCompactProps) {
  ...
  return (
    <Card className={`w-full max-w-xs mx-auto bg-gradient-to-br from-blue-300 to-indigo-400 border border-blue-300 shadow-lg print:shadow-none ${isArabic ? 'rtl' : 'ltr'}`}>
          ...
          <div className="grid grid-cols-1 gap-1 text-xs">
            {card.guest.nationalId && (
              <div className="flex justify-between">
                <span className="text-gray-500 font-medium">{isArabic ? 'الهوية:' : 'ID:'}</span>
                <span className="text-gray-700 truncate">{card.guest.nationalId}</span>
              </div>
            )}
            {card.guest.nationality && (
              <div className="flex justify-between">
                <span className="text-gray-500 font-medium">{isArabic ? 'الجنسية:' : 'Nationality:'}</span>
                <span className="text-gray-700 truncate">{card.guest.nationality}</span>
              </div>
            )}
            {card.guest.company && (
              <div className="flex justify-between">
                <span className="text-gray-500 font-medium">{isArabic ? 'الشركة:' : 'Company:'}</span>
            ...
            {card.guest.roomNumber && (
              <div className="flex justify-between">
                <span className="text-gray-500 font-medium">{isArabic ? 'الغرفة:' : 'Room:'}</span>
